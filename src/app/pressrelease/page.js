...
function Page() {
  ...
  const fetchMedias = async () => {
    try {
      const result = await axios.get("http://localhost:4000/getpress")
    ...
    <>
      ...
      <div className=' p-10 grid grid-cols-1 md:grid-cols-4 gap-y-24 gap-x-8 my-8'>
        ...
        {allData.map((item) => (<div key={item._id}>
          {console.log(item)}
          <PressCard title={item.title} link={item.link} text={item.text} alt={item.filename} img={item.filename} id={item._id} isAdmin={isAdmin} />
